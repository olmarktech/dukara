...
Route::get("assets/theme/screenshot/{theme}", function ($theme){
    try {
        // Always return placeholder to avoid 404 errors that break navbar
        $placeholder = public_path('no-image.jpeg');
        
        $themeData = renderPrimaryThemeScreenshot($theme);
        
        if (!empty($themeData)) {
            $image_name = last(explode('/',$themeData));
            $filePath = str_replace('/assets','/screenshot', theme_assets($image_name, $theme));
            ...
            $altPaths = [
                public_path("themes/{$theme}/screenshot/{$image_name}"),
                public_path("themes/{$theme}/assets/images/screenshot.png"),
            ];
        ...
        // If no placeholder, return 1px transparent image
        return response()->make(base64_decode('R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7'), 200, [
            'Content-Type' => 'image/gif',
        ]);
        
    } catch (\Exception $e) {
        \Log::error("Theme screenshot error for {$theme}: " . $e->getMessage());
        $placeholder = public_path('no-image.jpeg');
        if (file_exists($placeholder)) {
            return response()->file($placeholder);
        }
