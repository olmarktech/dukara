...
-------------------------------------------- */
    ...
    Route::controller(PaymentSettingsController::class)->name('admin.payment.settings.')->prefix('payment-settings/payment')->group(function (){
        ...
        Route::get('/kineticpay', 'kineticpay_settings')->name('kineticpay');
        ...
        Route::get('/awdpay', 'awdpay_settings')->name('awdpay');
        Route::get('/sslcommerz', 'sslcommerz_settings')->name('sslcommerz');
    ...
    Route::get("assets/css/{theme_name}/{file_path}", function ($theme_name,$file_path){

        $file_path = str_replace('__','/',$file_path);
        $path = public_path("../../core/resources/views/admin-themes/{$theme_name}/{$file_path}");
        if(!is_dir($path .'.css') && file_exists($path .'.css') ){
            return response()->file($path.'.css',['Content-Type' => 'text/css']);
        }

        return abort(404);
    })->name("admin.theme.css.file.url");



    Route::get("assets/js/{theme_name}/{file_path}", function ($theme_name,$file_path){

        $file_path = str_replace('__','/',$file_path);
        $path = public_path("../../core/resources/views/admin-themes/{$theme_name}/{$file_path}");

        if(!is_dir($path .'.js') && file_exists($path .'.js') ){
            return response()->file($path.'.js',['Content-Type' => 'application/javascript']);
        }

        return abort(404);
    })->name("admin.theme.js.file.url");
...
//    })->name("admin.theme.js.file.url");
    ...
    Route::get("assets/image/{theme_name}/{image_path}", function ($theme_name,$image_path) {
        $path = public_path("../../core/resources/views/admin-themes/{$theme_name}/{$image_path}");
//dd($path);
        if (!file_exists($path)) {
            abort(404);
        }
    ...
    })->name("admin.theme.image.file.url");
