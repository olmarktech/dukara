...
class BlogStyleOne extends PageBuilderBase
{
    ...
    public function frontend_render()
    {
        ...
        // For blog listing pages, always use pagination for better UX
        // If items is set and less than 20, use take, otherwise paginate
        if (!empty($items) && $items < 20)
        {
            $blogs = $blogs->orderBy($order_by,$order)->take($items)->get();
        } else {
            $items_per_page = !empty($items) && $items >= 20 ? $items : 6;
            $blogs = $blogs->orderBy($order_by,$order)->paginate($items_per_page);
        }
    ...
    {
        return __('Blog Posts Grid (Home/Pricing/Contact Page)');
    }
}
