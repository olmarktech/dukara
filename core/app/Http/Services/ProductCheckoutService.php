...
class ProductCheckoutService
{
    public function getOrCreateUser($validated_data): array
    {
        ...
        if (!$user) {
            ...
            $state_id = $validated_data['state'] ?? 0;
...
//                $regular_price = calculatePrice($data->options->regular_price, $data->options);
    ...
    public function getFinalPriceDetails($user, $validated_data)
    {
        ...
        if (array_key_exists('PHYSICAL', $price)) {
            ...
            if (is_array($discounted_price) && isset($discounted_price['success']) && $discounted_price['success'] === false) {
                session()->flash('coupon_error', $discounted_price['message']);
                $discount = 0;
            } elseif (is_array($discounted_price) && isset($discounted_price['total']) && $discounted_price['total'] != 0) {
                $discount = $discounted_price['total'];
            } else {
                $discount = is_numeric($discounted_price) ? $discounted_price : 0;
            }
            if ($discount > 0) {
    ...
    public function createOrder($validated_data, $user)
    {
        ...
        if (!empty($coupon['coupon']) && !empty($order_id) && ProductOrder::find($order_id)) {
            CouponUsage::create([
                'user_id' => $user['id'],
