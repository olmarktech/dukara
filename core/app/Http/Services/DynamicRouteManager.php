...
class DynamicRouteManager
{
    ...
    private static function handleLandlordPage($slug)
    {
        ...
        if ($active_theme === 'lexend-v4' && view()->exists('themes.lexend-v4.pages.dynamic-single')) {
            ...
            if ($slug === $price_page_slug) {
                ...
                return view('themes.lexend-v4.pages.dynamic-single')->with([
                    'all_blogs' => $all_blogs,
                    'page_post' => $page_post
                ]);
            }
    ...
    private static function handleTenantPage($slug)
    {
        ...
        // Always use the tenant's default dynamic-single page for tenants
        // to avoid theme compatibility issues with lexend-v4
        if (tenant()) {
            return view('tenant.frontend.pages.dynamic-single')->with([
                'page_post' => $page_post
            ]);
        }

        // For landlord, use themeView
