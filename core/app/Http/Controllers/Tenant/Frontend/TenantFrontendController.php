...
class TenantFrontendController extends Controller
{
    ...
    public function homepage()
    {
        ...
        // Only set meta data if page exists
        if ($page_post) {
            $this->setMetaDataInfo($page_post);
        }

        // Use theme-specific home view
        $data = [];
        if ($page_post && $page_post->exists) {
            $data['page_post'] = $page_post;
        }

        // Try direct view path to avoid themeView issues
        return view('themes.lexend-v4.pages.home')->with($data);
    }
...
//        return response()->json(['type' => 'danger', 'coupon_amount' => 0]);
        $coupon_result = CheckoutCouponService::calculateCoupon($request, $subtotal, $products, 'DISCOUNT');

        if (is_array($coupon_result) && !$coupon_result['success']) {
            return response()->json([
                'type' => 'error',
                'msg' => __($coupon_result['message']),
                'coupon_amount' => 0
            ]);
        }

        $discountedSubtotal = is_array($coupon_result) ? $coupon_result['total'] : $coupon_result;
        $data = $this->get_product_shipping_tax_for_coupon([
            'country' => $request->country,
            'state' => $request->state,
        ...
        $shipping_cost = $data['shipping_cost'];
        $taxed_price = ((double)$subtotal * $product_tax) / 100;
        $total = $discountedSubtotal + $taxed_price + $shipping_cost;
        return response()->json([
            'type' => 'success',
            'coupon_amount' => round($discountedSubtotal, 2),
            'coupon_price' => round((double)$subtotal - $discountedSubtotal, 2),
            'total' => round($total, 2),
            'msg' => __('Coupon applied successfully')
        ]);
