...
//        dd($request->all());
        ...
        // hash:make password
        $password = Hash::make($request->database_user_password);

        $id = $request->exception_id;
        $manual_database = trim($request->database_name);
        $manual_database_user_name = trim($request->database_user_name);
        $manual_database_user_pass = $password;

        $exception = TenantException::findOrFail($id);
        $tenant = Tenant::find($exception->tenant_id);

//        dd($manual_database, $manual_database_user_name, $manual_database_user_pass);
        if(is_null($tenant)){
        ...
        try{
            ...
            $format['tenancy_db_username'] = $manual_database_user_name;
            $format['tenancy_db_password'] = $manual_database_user_pass;
