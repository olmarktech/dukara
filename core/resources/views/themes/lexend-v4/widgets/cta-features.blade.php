            ...
            <div class="section-inner panel p-4 sm:p-6 xl:p-8 rounded-2 bg-secondary dark:bg-gray-800 overflow-hidden">
                <div class="vstack gap-2 max-w-550px items-center lg:items-start m-auto lg:m-0 text-center lg:text-start rtl:lg:text-end">
                    ...
                    <div class="vstack sm:hstack justify-center lg:justify-start gap-1 lg:gap-2 mt-1 lg:mt-2">
                        <a href="{{ $button_url }}" class="btn btn-md btn-primary text-white">{{ $button_text }}</a>
                    </div>
                    @if($button_subtext)
                        <p class="fs-7 text-dark dark:text-white text-opacity-70">{{ $button_subtext }}</p>
                    @endif
                </div>
                @if(!empty($image_light) || !empty($image_dark))
                    <div class="position-absolute top-50 ltr:end-0 rtl:start-0 translate-middle-y z-1 ltr:me-8 rtl:ms-8 d-none lg:d-block">
                        @if(!empty($image_light))
                            <img class="w-250px xl:w-300px d-block dark:d-none ltr:xl:me-7 rtl:xl:ms-7" src="{{ $image_light }}" alt="charts">
                        @endif
                        @if(!empty($image_dark))
                            <img class="w-250px xl:w-300px d-none dark:d-block ltr:xl:me-7 rtl:xl:ms-7" src="{{ $image_dark }}" alt="charts-dark">
                        @endif
                    </div>
                @else
                    <div class="position-absolute top-50 ltr:end-0 rtl:start-0 translate-middle-y z-1 ltr:me-8 rtl:ms-8 d-none lg:d-block">
                        <img class="w-250px xl:w-300px d-block dark:d-none ltr:xl:me-7 rtl:xl:ms-7" src="{{ asset('themes/lexend-v4/assets/images/template/charts.svg') }}" alt="charts">
                        <img class="w-250px xl:w-300px d-none dark:d-block ltr:xl:me-7 rtl:xl:ms-7" src="{{ asset('themes/lexend-v4/assets/images/template/charts-dark.svg') }}" alt="charts-dark">
                    </div>
                @endif
            </div>
        </div>
