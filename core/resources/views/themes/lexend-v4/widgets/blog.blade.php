@php
    // Blog section - EXACT from index-13.html line 1605-1720
    $title = $data['title'] ?? '';
@endphp
                                            ...
                                            <div class="panel vstack justify-between gap-2 p-1 pt-2 xl:p-2">
                                                <div class="content vstack gap-1 xl:gap-2">
                                                    @if(!empty($blog->category) && !empty($blog->category->id))
                                                        <a href="{{ route('landlord.frontend.blog.category', [$blog->category->id, $blog->category->slug ?? '']) }}" class="post-excrept fs-7 text-uppercase text-none text-gray-300 dark:text-gray-200">{{ $blog->category->title }}</a>
                                                    @endif
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const wrapper = blogSwiper.querySelector('.swiper-wrapper');
    if (wrapper) {
    ...
    if (window.UIkit && UIkit.swiper) {
        setTimeout(() => {
            try {
                ...
                console.log('Blog swiper initialized via UIkit');
            } catch(e) {
    ...
    // Fallback: Manual swiper initialization if UIkit fails
    setTimeout(() => {
        const wrapper = blogSwiper.querySelector('.swiper-wrapper');
        if (wrapper && wrapper.style.transform === '') {
            console.log('Initializing blog swiper manually');
            wrapper.style.display = 'flex';
