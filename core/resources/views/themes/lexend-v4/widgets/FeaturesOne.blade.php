                                ...
                                <div>
                                    <div class="feature-item panel overflow-hidden p-2 bg-white border border-1 dark:bg-opacity-5 dark:text-white rounded-2 xl:rounded-3">
                                        <div class="panel vstack items-start gap-1 xl:gap-2 p-1 pb-2 lg:p-3 xl:p-4">
                                            <h4 class="h4 xl:h3 m-0 text-inherit">{{ $data['repeater_data']['repeater_title_'][$key] }}</h4>
                                            <p class="fs-6 xl:fs-5 text-gray-400 dark:text-gray-200">{{ $data['repeater_data']['repeater_description_'][$key] }}</p>
                                        </div>
                                        <div class="panel">
                                            @if(!empty($data['repeater_data']['repeater_image_'][$key]))
                                                {!! render_image_markup_by_attachment_id($data['repeater_data']['repeater_image_'][$key], $data['repeater_data']['repeater_title_'][$key], 'full', false, ['class' => 'rounded-2']) !!}
                                            @endif
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="section-footer panel vstack gap-2 items-center mt-6 sm:mt-8 xl:mt-9" data-anime="translateY: [24, 0]; opacity: [0, 1]; easing: easeOutExpo; duration: 750; delay: 750;">
                    @if(!empty($data['cta_button_text']) && !empty($data['cta_button_url']))
                        ...
                        @if(!empty($data['cta_button_subtext']))
                            <span class="fs-7 dark:text-white text-opacity-75">{{ $data['cta_button_subtext'] }}</span>
                        @endif
                    @endif
                </div>
            </div>
