                ...
                <div class="row">
                    <div class="col-lg-3">
                        <ul>
                            ...
                            <li><strong>{{__('Registered:').' '}}</strong> {{$user->created_at->format('d M Y')}} <small>({{$user->created_at->diffForHumans()}})</small></li>
                        </ul>
                    </div>
                    <div class="col-lg-9">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @php
                                               ...
                                               if (!empty($payment))
                                               {
                                                   $start_date = date('d-m-Y',strtotime($tenant->payment_log->start_date));
                                                   $end_date = date('d-m-Y',strtotime($tenant->payment_log->expire_date));;
                                               }
                                                $start_date = date('d-m-Y',strtotime($tenant->payment_log->start_date));
                                               $end_date = $tenant?->expire_date != null ? date('d-m-Y',strtotime($tenant->payment_log->expire_date)) : __('Lifetime');
                                            @endphp
                                            ...
                                            <span class="d-block mb-2">
                                                @php
                                                    if ($tenant->payment_log->expire_date != null)
                                                    {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        $trial_status =  $tenant?->payment_log->expire_date != null ? get_price_plan_expire_status($end_date) : ($end_date !== 'Lifetime' ? 'trial' : __('Lifetime'));
                                                    }
...
{{--                                                @dd($log);--}}
                                                <span class="{{$class[$status]}} text-capitalize">
                                                        ...
                                                        @if($status != 'active' && $status != 'expired')
                                                            @if($status == 'trial')
                                                                {{$status}} - <span class="{{$class[$trial_status]}}">{{$trial_status}}</span>
                                                            @else
                                                                {{__('pending')}}
                                                            @endif
                                                        @else
                                        ...
                                        <td>{{$tenant->payment_log->renew_status ?? 0}}</td>
                                        <td>
