...
//        dd($data);
    ...
    public function prepareProductInventoryDetailsAndInsert($data, $product_id, $inventory_id, $type="create"): bool
    {
        ...
        // Check if inventory details are valid before processing
        if (!$this->hasValidInventoryDetails($data)) {
            return true; // Return true but don't insert any inventory details
        }

        // count item stock for getting array length
        $len = count($data["item_stock_count"] ?? []); //[1,2,3] count ([]) = null

        for($i = 0;$i < $len; $i++){
            // Check if this specific item has valid data
            $hasValidItemData = false;

            $fieldsToCheck = [
                'item_size', 'item_color', 'item_image',
                'item_additional_price', 'item_extra_cost'
            ];

            foreach ($fieldsToCheck as $field) {
                if (!empty($data[$field][$i]) && $data[$field][$i] !== null) {
                    $hasValidItemData = true;
                    break;
                }
            }

            // Check attributes for this item
            if (!$hasValidItemData && !empty($data["item_attribute_name"][$i])) {
                foreach ($data["item_attribute_name"][$i] as $attrName) {
                    if (!empty($attrName) && $attrName !== null) {
                        $hasValidItemData = true;
                        break;
                    }
                }
    ...
    {
        // Check if inventory details have meaningful data
        $useItemStockCount = false;

        if (!empty($data["item_stock_count"]) && is_array($data["item_stock_count"])) {
            $len = count($data["item_stock_count"]);

            for ($i = 0; $i < $len; $i++) {
                // Check if ANY of these fields have non-null values
                if (
                    (!empty($data["item_size"][$i]) && $data["item_size"][$i] !== null) ||
                    (!empty($data["item_color"][$i]) && $data["item_color"][$i] !== null) ||
                    (!empty($data["item_image"][$i]) && $data["item_image"][$i] !== null) ||
                    (!empty($data["item_additional_price"][$i]) && $data["item_additional_price"][$i] !== "0") ||
                    (!empty($data["item_extra_cost"][$i]) && $data["item_extra_cost"][$i] !== "0")
                ) {
                    $useItemStockCount = true;
                    break;
                }

                // Check attributes
                if (!empty($data["item_attribute_name"][$i])) {
                    foreach ($data["item_attribute_name"][$i] as $attrName) {
                        if (!empty($attrName) && $attrName !== null) {
                            $useItemStockCount = true;
                ...
                if (!empty($data["item_attribute_value"][$i])) {
                    foreach ($data["item_attribute_value"][$i] as $attrValue) {
    ...
    private function hasValidInventoryDetails($data): bool
    {
        // Check if we have item_stock_count
        if (empty($data["item_stock_count"]) || !is_array($data["item_stock_count"])) {
            return false;
        }
        ...
        // Check each inventory item to see if it has valid details
        for ($i = 0; $i < $len; $i++) {
            $hasValidData = false;

            // Check if any of these fields have valid (non-null/non-empty) values
            $fieldsToCheck = [
                'item_size', 'item_color', 'item_image',
                'item_additional_price', 'item_extra_cost'
            ];
            ...
            // Check attributes
            if (!$hasValidData && !empty($data["item_attribute_name"][$i])) {
                foreach ($data["item_attribute_name"][$i] as $attrName) {
                    if (!empty($attrName) && $attrName !== null) {
                        $hasValidData = true;
                        break;
            ...
            if (!$hasValidData && !empty($data["item_attribute_value"][$i])) {
                foreach ($data["item_attribute_value"][$i] as $attrValue) {
                    if (!empty($attrValue) && $attrValue !== null) {
                        $hasValidData = true;
                        break;
            ...
            // If we found at least one item with valid data, return true
            if ($hasValidData) {
                return true;
            }
        }
...
//            dd($this->prepareProductInventoryDetailsAndInsert($data, $id, $inventory->id));
        }
    ...
    protected function productInstance($type): Builder
    {
        ...
        if($type == "edit"){
            $product->with(["product_category","tag","uom","product_sub_category","product_child_category","inventory","delivery_option","custom_specifications"]);
        }elseif ($type == "single"){
            $product->with(["category","gallery_images","tag","uom","subCategory","childCategory","image","inventory","delivery_option","custom_specifications"]);
        }elseif ($type == "list"){
            $product->with(["category","uom" , "subCategory", "childCategory", "brand", "badge" , "image" , "inventory","custom_specifications"]);
        }elseif ($type == "search"){
            $product->with(["category","uom" , "subCategory", "childCategory", "brand", "badge" , "image" , "inventory","custom_specifications"]);
        }else{
            $product = Product::query()->with(["category" , "subCategory", "childCategory", "brand", "badge" , "image" , "inventory","custom_specifications"]);
        }
...
//        dd($product);
    ...
    public function productUpdate($data, $id){
        ...
        $inventoryDetail = false;
        //todo:: check product single page while order a product using the variant, it's showing out of stock or not

//        dd()
//        if(!empty($data["item_stock_count"][0])){
//        dd($data, $id,$product?->inventory,"update");
